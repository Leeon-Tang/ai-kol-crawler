...
const AcademicDataTab = memo(({ isActive, stats }: AcademicDataTabProps) => {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['github-academic-data', currentPage, pageSize, statusFilter],
    queryFn: () => apiService.getPlatformData('github_academic', currentPage, pageSize, statusFilter === 'all' ? undefined : statusFilter),
    enabled: isActive,
    staleTime: 30000,
  })
  ...
  const exportMutation = useMutation({
    mutationFn: () => apiService.exportData('github_academic', 'xlsx'),
    onSuccess: () => {
      // 触发下载
    },
  })
      ...
      <div className="data-header-compact">
        <div className="header-left">
          <h2 className="data-title">
            <GithubOutlined /> GitHub 学术人士
          </h2>
          <div className="stats-inline">
            <span className="stat-item">
              <span className="stat-label">总计</span>
              <span className="stat-value">{stats?.total_academic_developers?.toLocaleString() || 0}</span>
            </span>
            <span className="stat-item">
              <span className="stat-label">合格</span>
              <span className="stat-value success">{stats?.qualified_academic_developers?.toLocaleString() || 0}</span>
            </span>
            <span className="stat-item">
              <span className="stat-label">合格率</span>
              <span className="stat-value">
                {stats?.total_academic_developers ? Math.round((stats.qualified_academic_developers / stats.total_academic_developers) * 100) : 0}%
              </span>
            </span>
          </div>
        </div>
...
AcademicDataTab.displayName = 'AcademicDataTab'

export default AcademicDataTab
