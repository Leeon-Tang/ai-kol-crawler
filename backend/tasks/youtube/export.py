...
class YouTubeExportTask:
    ...
    def run_today(self, today_start, today_end):
        """
        导出今日KOL到Excel
        ...
        # 查询今日数据
        query = """
            SELECT * FROM youtube_kols 
            WHERE status = 'qualified'
            AND discovered_at >= ? AND discovered_at <= ?
            ORDER BY ai_ratio DESC
        """
        kols = self.repository.db.fetchall(query, (start_str, end_str))
        
        if not kols:
            logger.info("今天没有合格的KOL可导出")
            return ""
        
        logger.info(f"待导出今日KOL数: {len(kols)}")
        
        # 创建Excel
        wb = Workbook()
        ws = wb.active
        ws.title = "AI KOL列表(今日)"
        ...
        filename = f"kol_report_today_{timestamp}.xlsx"
        filepath = os.path.join(output_dir, filename)
