...
class Database:
    ...
    def __init__(self, db_path=None):
        ...
        if not os.path.exists(self.db_path):
            print(f"[INFO] 数据库文件不存在，将创建新数据库: {self.db_path}")
    ...
    def connect(self):
        ...
        try:
            ...
            # 如果是新数据库，初始化表结构
            if not db_exists:
                print(f"[INFO] 初始化新数据库表结构...")
                self.init_tables()
                print(f"[INFO] 数据库初始化完成: {self.db_path}")
            
            print(f"[INFO] 数据库连接成功: {self.db_path}")
            
        except Exception as e:
            print(f"[ERROR] 数据库连接失败: {e}")
            print(f"[ERROR] 数据库路径: {self.db_path}")
            raise
