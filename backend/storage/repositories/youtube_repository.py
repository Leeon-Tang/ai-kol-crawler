...
class YouTubeRepository:
    ...
    def get_kols_paginated(self, page=1, page_size=20, status=None):
        """分页获取KOL列表"""
        ...
        # 获取总数
        count_query = f"SELECT COUNT(*) as count FROM youtube_kols {where_clause}"
        total_result = self.db.fetchone(count_query)
        ...
        data_query = f"""
            ...
            LIMIT ? OFFSET ?
        """
        items = self.db.fetchall(data_query, (page_size, offset))
